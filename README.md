...
- 也可強制切換為純靜態或純 AI 模式
- **Power Automate 08:00 準時排程** + GitHub Actions 執行（不依賴 GitHub cron）
...
├── requirements.txt
├── .github/workflows/teams_bot.yml      # 晨間問候（由 PA 觸發 workflow_dispatch）
├── docs/power_automate_schedule.md      # PA 08:00 排程設定說明
└── README.md
```
...
- 成功後 Teams 頻道會收到訊息
- 設定 Power Automate 排程後，每週一至五 **08:00 台灣時間** 自動發送（見下方「排程說明」）

### 步驟 4：設定 Power Automate 08:00 排程（必做）

GitHub cron 已停用（延遲可達數小時）。請在 Power Automate 建立定時 flow 觸發 GitHub Actions：

👉 完整步驟見 **[docs/power_automate_schedule.md](docs/power_automate_schedule.md)**

摘要：
1. 建立 **Scheduled cloud flow**（週一至五 08:00，時區台北）
2. 新增 **HTTP POST** 到 GitHub `workflow_dispatch` API
3. 使用具 **Actions: Write** 權限的 GitHub PAT

本機測試觸發：`.\trigger_morning_bot.bat`
...
| 元件 | 角色 |
|------|------|
| **Power Automate** | 每週一至五 **08:00（台北）** 準時觸發 |
| **GitHub Actions** | 執行 `main.py`（讀 Secrets、AI、圖片） |
| **Send webhook alerts to HK-ALL** | 收到 webhook → 發到 Teams 頻道 |

`teams_bot.yml` **不再使用 GitHub cron**（避免延遲 1～5 小時）。  
設定教學：[docs/power_automate_schedule.md](docs/power_automate_schedule.md)

假日判斷由 Python 腳本在執行時處理；PA 在國定假日若仍觸發，程式也會自動跳過發送。
